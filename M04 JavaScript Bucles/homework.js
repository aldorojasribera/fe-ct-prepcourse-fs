...
function esPrimo(num) {
   ...
   if (num <= 1) {
    ...
    for (let i = 2; i <= Math.sqrt(num); i++) {  //Math.sqrt() calcula la raiz cuadrada
      if (num % i === 0) {
        return false; // Si el número es divisible por algún número distinto de 1 y sí mismo, no es primo.
      }
    }
  
    return true; // Si el número no fue divisible por ningún número distinto de 1 y sí mismo, es primo.
}
esPrimo(16)
